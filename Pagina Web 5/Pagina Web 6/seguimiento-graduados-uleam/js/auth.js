...
/*
  ...
  Mejoras:
  - Redirigir al dashboard específico según rol (egresado/coordinador)
  - Proteger y poblar las páginas `dashboard-coordinador.html` y `dashboard-egresado.html`
  - Mantener el dashboard genérico `dashboard.html` como alternativa
*/
(function () {
  ...
  if (regForm) {
    ...
    regForm.addEventListener('submit', function (e) {
      ...
      const validators = [
        ...
        // Validación de correo: ahora se requiere @gmail.com
        { ok: !!email && isValidEmail(email), el: q('email'), msg: 'Correo inválido (debe terminar en @gmail.com).' },
  ...
  // (removed development helper: forceSeedSurveys)
    ...
    document.body.addEventListener('click', function delegatedAppActions(e){
      try {
        ...
        const empSaveBtn = e.target.closest && e.target.closest('#empSave');
        if (empSaveBtn) {
          const sess = JSON.parse(sessionStorage.getItem(SESSION_KEY) || 'null');
          if (!sess) return alert('Debe iniciar sesión');
          const email = sess.email;
